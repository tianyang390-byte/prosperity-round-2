...
class SimulatedExchange:
    ...
    def execute_orders(self, orders: List[Order], order_depths: Dict[str, OrderDepth], timestamp: int) -> float:
        for order in orders:
            ...
            if qty > 0:
                if od.sell_orders:
                    ...
                    if exec_qty > 0:
                        ...
                        self.trades.append({'ts': timestamp, 'action': 'BUY', 'price': best_ask, 'qty': exec_qty})
            elif qty < 0:
                if od.buy_orders:
                    ...
                    if exec_qty > 0:
                        ...
                        if abs(new_position) <= self.position_limit:
                            ...
                            self.trades.append({'ts': timestamp, 'action': 'SELL', 'price': best_bid, 'qty': exec_qty})
        return 0.0
...
class MomentumStrategyWithProfitTaking:
    """趋势策略+固定止盈"""

    def __init__(self, profit_take_pct=0.025, stop_loss_pct=0.015, trailing_stop_pct=0.025, lookback=20):
        self.position_limit = 80
        self.profit_take_pct = profit_take_pct  # 固定止盈 %
...
def run_strategy(intarian_data, profit_take_pct, stop_loss_pct, trailing_stop_pct, lookback):
    exchange = SimulatedExchange()
    exchange.reset()
    momentum = MomentumStrategyWithProfitTaking(profit_take_pct, stop_loss_pct, trailing_stop_pct, lookback)
    ...
    for timestamp in all_timestamps:
        ...
        exchange.execute_orders(orders, order_depths, timestamp)

    return exchange.get_final_value(intarian_data), exchange.trades
...
def main():
    ...
    for profit_take in [0.015, 0.020, 0.025, 0.030, 0.035]:
        pnls = []
        total_trades = 0
        for folder_name, intarian_data in all_data:
            pnl, trades = run_strategy(intarian_data, profit_take, 0.015, 0.025, 20)
            pnls.append(pnl)
            total_trades += len([t for t in trades if t['action'] == 'SELL'])
        avg_pnl = sum(pnls) / len(pnls)
        print(f"ProfitTake={profit_take*100:.1f}%: AvgPnL={avg_pnl:.2f}, SELL次数={total_trades}")

    print("\n对比：无止盈（原始策略）")
    pnls = []
    for folder_name, intarian_data in all_data:
        pnl, trades = run_strategy(intarian_data, 999.0, 0.015, 0.025, 20)  # 永远不会触发
        pnls.append(pnl)
    print(f"无止盈: AvgPnL={sum(pnls)/len(pnls):.2f}")
